...
public class RockerView extends View {
    ...
    private OnTouchPointListener mOnTouchPointListener;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.v(TAG, "onMeasure: --------------------------------------");
        Log.v(TAG, "onMeasure: widthMeasureSpec = " + widthMeasureSpec + " heightMeasureSpec = " + heightMeasureSpec);
        Log.v(TAG, "onMeasure: widthMode = " + widthMode + "  measureWidth = " + widthSize);
        Log.v(TAG, "onMeasure: heightMode = " + heightMode + "  measureHeight = " + widthSize);
        Log.v(TAG, "onMeasure: measureWidth = " + measureWidth + " measureHeight = " + measureHeight);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mAreaBackgroundPaint.setColor(mGrayAreaColor);
        canvas.drawCircle(mCenterPoint.x, mCenterPoint.y, mAreaRadius, mAreaBackgroundPaint);
        mRockerPaint.setColor(mGrayRockerColor);
        canvas.drawCircle(mCenterPoint.x, mCenterPoint.y, mRockerRadius, mRockerPaint);
    }
...
//                Log.d(TAG, "onTouchEvent: 抬起位置 : x = " + upX + " y = " + upY);
    ...
    private Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) {
        ...
        // 两点在Y轴距离
        float lenY = (float) (centerPoint.y - touchPoint.y);
        // 两点距离
...
//        Log.d(TAG, "getRockerPositionPoint: " + radian + " " + angle);

        // 回调 返回参数

        // 计算当前触屏点y轴的比例值
        float pery = calcPointScale(lenY, rockerRadius, regionRadius);

        // 计算弧速比例值
        float perradian = calcRadScale((float) radian);

        callBack(angle, pery, perradian);

//        Log.d(TAG, "getRockerPositionPoint: 角度 :" + angle);
    ...
    private float calcPointScale(float v, float rockerRadius, float regionRadius) {
        float per = 0.0f;
        if (Math.abs(v) + rockerRadius <= regionRadius) {
            per = v / (regionRadius - rockerRadius);
        } else {
            if (v > 0) {
                per = 1;
            } else {
    ...
    private float calcRadScale(float rad) {
        float per = 0.0f;

//        LogUtil.d(TAG, "current rad: " + rad);

        // 左半边
        if (rad >= Math.PI / 2 && rad <= Math.PI) {
            per = (float) ((rad / 2) / (Math.PI / 2)) * -1;
        } else if (rad >= -Math.PI && rad <= -Math.PI / 2) {
            per = (float) ((-Math.PI / 2 - rad) / (Math.PI / 2)) * -1;
        }

        // 右半边
        if (rad >= 0 && rad < Math.PI / 2) {
            per = (float) ((Math.PI / 2 - rad) / (Math.PI / 2));
        } else if (rad > -Math.PI / 2 && rad < 0) {
            per = (float) ((rad + Math.PI / 2) / (Math.PI / 2));
        }
...
//        Log.d(TAG, "onTouchEvent: 移动位置 : x = " + mRockerPosition.x + " y = " + mRockerPosition.y);
     ...
     * @param angle 摇动角度
     */
    private void callBack(double angle, float y, float radian) {
        if (null != mOnAngleChangeListener) {
    ...
    private void callBackFinish() {
        ...
        if (mOnTouchPointListener != null) {
            mOnTouchPointListener.onFinish();
        }
    }
    ...
    public void setOnTouchPointListener(OnTouchPointListener listener) {
        mOnTouchPointListener = listener;
    }
